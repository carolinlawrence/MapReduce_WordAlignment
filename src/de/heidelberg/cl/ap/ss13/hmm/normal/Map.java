...
public class Map extends Mapper<LongWritable, Text, Text, MapWritable> {
	...
	private void backward(String[] states, String[] observations){
		...
		//initalisation
		for(int q = 0; q < states.length; q++){
			beta[observations.length-1][q] = 1.0;//observations.length-1 because counting starts at 0
		}
	...
	/**
	 * loads the hmm (translation table[=emissions probabiliy], transition probability and the probability for the initial state) for this Mapper for the current iteration before any map function calls
	 *
			...
			for(int x = 0; x < observations.length; x++){
				for(int q = 0; q < states.length; q++){
					...
					if(emission_new[q].containsKey(text_writable)){//not tested
						...
						double_writable.set(double_writable.get() + double_value);
